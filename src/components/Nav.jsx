...
const Nav = ({ logo, logoText }) => {
  ...
  return (
    <div className="relative flex items-center justify-between border-b-[0.5px] border-[#75687E] px-4 py-4 md:px-[3.5rem]">
      {/* Logo Section */}
      <div className="flex items-center">
        <img src={logo} alt="Logo" className="mr-2 h-10" />
        <p className="text-lg font-bold">{logoText}</p>
      </div>
      ...
      <div className="hidden w-auto items-center justify-between gap-14 md:flex">
        <ul className="flex space-x-4">
          {navItems.map((item, index) => (
            <li key={index} className="cursor-pointer text-base font-medium hover:text-[#8300FF]">
              {item}
            </li>
          ))}
        </ul>
      ...
      <div className="flex items-center md:hidden">
        <button onClick={toggleMobileMenu} className="text-3xl">
          <IoMenuSharp /> {/* Use the icon directly in JSX */}
        </button>
      </div>
